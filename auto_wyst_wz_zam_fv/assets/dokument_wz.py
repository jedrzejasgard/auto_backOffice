...
class DokumentWZ:
    ...
    * zamkniety -> 
    ...
    def wz_uwagi_do_fakturzystek(self):
        for wd in self._query['PolaUzytkownika']:
            if wd['NazwaWewnetrzna'] == 'uwagi_dla_fakturzystek_wz':
                if wd['Wartosc']:
                    return False
                else:
                    return True
    ...
    @property
    def pomijany_klienta(self):
        klienci_bez_autoFV=['9301']
        id_klienta = self._query['Klient1ID']
        czy_pomijany_klient = False
        if id_klienta in klienci_bez_autoFV:
            czy_pomijany_klient = True
        return czy_pomijany_klient