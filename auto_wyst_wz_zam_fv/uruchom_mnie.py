...
def main():
    if datetime.now().hour == 18 and datetime.now().minute < 50:
        print('dodaje koszty')
        dodanie_kosztow()
    # Lista dokumentów zamkniętych
    if datetime.now().hour > 8 and datetime.now().hour < 19 :
        '''
        Skrypt odpala się tylko między godziną 9 oraz 20 żeby księgowość miała możliwość wprowadzenia nowych kursów walut
        '''
        ...
        if mozna_fakturowac_otwarte_wz(datetime.today(), GODZINA):
